...
export const RecoverySheet = ({ card, transcriptOpen, onTranscriptOpenChange, onClose }: RecoverySheetProps) => {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-20 mx-auto max-w-xl rounded-t-[28px] bg-paper px-5 pb-[max(24px,env(safe-area-inset-bottom))] pt-4 shadow-sheet ring-1 ring-black/10">
      ...
      <div className="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="text-2xl font-semibold tracking-tight text-ink">{card.title}</h2>
        </div>
        <button
          type="button"
          onClick={onClose}
          className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-black/5 text-ink/70"
          aria-label="关闭恢复卡片"
        >
          <X className="h-5 w-5" />
        </button>
      ...
      <div className="grid gap-3">
        <CardSection label="刚刚讲到" content={card.summary} />
        <CardSection label="现在要做什么" content={card.action} tone="action" />
        <CardSection label="你现在该接着听" content={card.resumePoint} />

        {card.keyPoints.length > 0 ? (
          <section className="rounded-2xl bg-white/75 p-4 ring-1 ring-black/5">
            <h3 className="mb-2 text-sm font-semibold text-ink/55">重点信息</h3>
...
const CardSection = ({ label, content, tone = "default" }: { label: string; content: string; tone?: "default" | "action" }) => (
  <section className={`rounded-2xl p-4 ring-1 ring-black/5 ${tone === "action" ? "bg-amber/20" : "bg-white/75"}`}>
